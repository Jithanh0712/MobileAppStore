...
class AppRecommendation:
    ...
    def create_emp_table():
        try:
            query = "CREATE TABLE IF NOT EXISTS emp (emp_id INT, emp_name TEXT, emp_city TEXT, emp_sal VARINT, emp_phone VARINT, PRIMARY KEY(emp_id));"
            AppRecommendation.cassandra_manager.execute_query(query)
            return {"message": "Tạo bảng emp thành công"}
        except Exception as e:
            return {"error": str(e)}
    @staticmethod
    def create_new_user(emp_id, emp_name, emp_city, emp_sal, emp_phone):
        try:
            query = "INSERT INTO emp (emp_id, emp_name, emp_city, emp_sal, emp_phone) VALUES ({}, '{}', '{}', {}, {})".format(emp_id, emp_name, emp_city, emp_sal, emp_phone)
            AppRecommendation.cassandra_manager.execute_query(query)
            return {"message": "Thêm user mới thành công"}
        except Exception as e:
            return {"error": str(e)}
    ...
    def delete_emp_by_id(emp_id):
        try:
            query = "DELETE FROM emp WHERE emp_id = {}".format(emp_id)
            AppRecommendation.cassandra_manager.execute_query(query)
